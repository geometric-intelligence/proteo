...
from proteo.data.complex import Cochain, Complex
from proteo.data.parallel import ProgressParallel


def pyg_to_simplex_tree(edge_index: Tensor, size: int):
    """Constructs a simplex tree from a PyG graph.

    Note that this does not create rank-2 cells nor features.
    It only puts the nodes and edges into a simplex tree form.
...
def build_tables(simplex_tree, size):
    ...
    This assigns IDs to simplices of different dimensions, which orders the
    simplices.
    ...
    Returns
    -------
    simplex_tables : list of lists
        Eg, simplex_tables[0] gives the list of simplices of dim 0.
    id_maps : list of dicts
        Eg, id_maps[0] gives the map from the node to its id.
        Eg, id_maps[1] gives the map from the edge to its id.
    """
    ...
    # Initialize the 0th element of these data structures with nodes
    simplex_tables[0] = [[v] for v in range(size)]
    id_maps[0] = {tuple([v]): v for v in range(size)}

    # Loop through all simplices and compute their dim by counting their nodes
    for simplex, _ in simplex_tree.get_simplices():
        dim = len(simplex) - 1  # by property of the simplex
        if dim == 0:
            continue  # because that case has been handled above

        # Assign this simplex the next unused ID
        next_id = len(simplex_tables[dim])  # how many cells of dim we have already counted
        id_maps[dim][tuple(simplex)] = next_id  # connect node ids to the simplex of edges
        simplex_tables[dim].append(simplex)  # put simplex-edgde
...
def extract_boundaries_and_coboundaries_from_simplex_tree(simplex_tree, id_maps, complex_dim: int):
    ...
    simplex_tree : gudhi.SimplexTree
        The simplex tree of the graph or complex.
    id_maps : list of dicts
        Eg, id_maps[0] gives the map from the node to its id.
        Eg, id_maps[1] gives the map from the edge to its id.
    complex_dim : int
    ...
    Returns
    -------
    boundaries_tables : list of lists
        Eg boundaries_tables[1] gives the list of nodes ids that are on boundary of each edge.
        Eg boundaries_tables[2] gives the list of all edges ids that are on boundary of each face.
    boundaries : list of dicts
        Eg boundaries[2] = {(0, 1, 2): [(0, 1), (0, 2), (1, 2)]}
    coboundaries : list of dicts
        Eg coboundaries[1] = {(0, 1): [(0, 1, 2)], (0, 2): [(0, 1, 2)], (1, 2): [(0, 1, 2)]}
    """
    ...
    for simplex, _ in simplex_tree.get_simplices():
        ...
        level_coboundaries = coboundaries[simplex_dim]  # is an empty dict on the first pass
        level_boundaries = boundaries[simplex_dim + 1]

        # Add the boundaries of the simplex to the boundaries table
        if simplex_dim > 0:
            boundaries_ids = [
                id_maps[simplex_dim - 1][boundary] for boundary in get_simplex_boundaries(simplex)
            ]
...
def build_adj(
    ...
    -----------
    boundaries: list of dicts
        Eg boundaries[dim][simplex] -> List[simplex] (the boundaries)
    coboundaries: list of dicts
        Eg coboundaries[dim][simplex] -> List[simplex] (the coboundaries)
    id_maps: dict
        simplex -> simplex_id

    Returns:
    --------
    all_shared_boundaries: list of lists
        Eg all_shared_boundaries[dim] -> List[simplex_id] (the shared boundaries,
        thus all_shared_boundaries[0] is empty,
        and  all_shared_boundaries[1] contains list of nodes that pairs of edges share as boundary),
        and  all_shared_boundaries[2] contains list of edges that pairs of faces share as boundary)
    all_shared_coboundaries: list of lists
        Eg all_shared_coboundaries[dim] -> List[simplex_id] (the shared coboundaries,
        thus all_shared_coboundaries[0] contains list of edges that pairs of nodes share as coboundary),
        and  all_shared_coboundaries[1] contains list of faces that pairs of edges share as coboundary),
        and  all_shared_coboundaries[2] is empty
    lower_indexes: list of lists
        lower_indexes[dim] -> List[List[simplex_id]]
        Eg lower_indexes[1] -> List of indexes of the pairs of edges that share a node as boundary
           lower_indexes[2] -> List of indexes of the pairs of faces that share an edge as boundary
    upper_indexes: list of lists
        Eg upper_indexes[dim] -> List[List[simplex_id]]
        Eg upper_indexes[0] -> List of indexes of the pairs of nodes that share an edge as coboundary
           upper_indexes[1] -> List of indexes of the pairs of edges that share a face as coboundary
           upper_indexes[2] -> Should be empty
    """
    ...
    all_shared_boundaries, all_shared_coboundaries = (
    ...
    for dim in range(complex_dim + 1):
        ...
        for simplex, id in id_maps[dim].items():
            ...
            if include_down_adj and dim < complex_dim and simplex in coboundaries[dim]:
                for coboundary1, coboundary2 in itertools.combinations(
                    coboundaries[dim][simplex], 2
                ):
                    id1, id2 = (
                        id_maps[dim + 1][coboundary1],
                        id_maps[dim + 1][coboundary2],
                    )
...
def construct_features(vx: Tensor, cell_tables, init_method: str) -> List:
    """Combines the features of the component nodes to initialise the cell features.
    ...
    features : list of torch.Tensor
        features[0]: tensor of features on nodes, shape=[n_nodes, dim]
        features[1]: tensor of features on edges, shape=[n_edges, dim]
        features[2]: tensor of features on faces, shape=[n_faces, dim]
    """
    features = [vx]
...
def generate_cochain(
    ...
    ----------
    dim : int
        Dimension for which we are building the cochain.
    x : torch.Tensor
        Features for the cells of that dimension.
    upper_indexes: list of lists
        Eg upper_indexes[dim] -> List[List[simplex_id]]
        Eg upper_indexes[0] -> List of indexes of the pairs of nodes that share an edge as coboundary
           upper_indexes[1] -> List of indexes of the pairs of edges that share a face as coboundary
    ...
    all_shared_coboundaries: list of lists
        ...
        and  all_shared_coboundaries[2] is empty
    complex_dim : int
        Total dimension of the complex (typically: 2)
    boundaries_tables : list of lists
        Eg boundaries_tables[1] gives the list of nodes ids that are on boundary of each edge.
        Eg boundaries_tables[2] gives the list of all edges ids
        that are on boundary of each face.
    """
    if dim == 0:
    ...
    # Get what is stored in the lists for that dim

    # print("len(all_upper_index)", len(all_upper_index))
    # print("len(all_lower_index)", len(all_lower_index))
    # print("len(all_shared_coboundaries)", len(all_shared_coboundaries))
    # print("len(all_shared_boundaries)", len(all_shared_boundaries))

    try:
        up_index = (
            torch.tensor(all_upper_index[dim], dtype=torch.long).t()
            if len(all_upper_index[dim]) > 0
            else None
        )
    except Exception as e:
        print("An error occurred:", e)
        print("all_upper_index len", len(all_upper_index))
        print("all_upper_index", all_upper_index)
        up_index = None

    down_index = (
        torch.tensor(all_lower_index[dim], dtype=torch.long).t()
        if len(all_lower_index[dim]) > 0
        else None
    )
    shared_coboundaries = (
        torch.tensor(all_shared_coboundaries[dim], dtype=torch.long)
        if len(all_shared_coboundaries[dim]) > 0
        else None
    )
    shared_boundaries = (
        torch.tensor(all_shared_boundaries[dim], dtype=torch.long)
        if len(all_shared_boundaries[dim]) > 0
        else None
    )
    ...
    # boundary index is two lists, where:
    # boundary[0]: list of edges ids in the boundary of that face
    # boundary[1]: list of new_ids of faces
    # for example:
    # boundary[0] = [1, 2, 3, 2, 0, 1]
    # boundary[1] = [0, 0, 0, 1, 1, 1]
...
def compute_clique_complex_with_gudhi(
    ...
    # This should provide cells of rank 2, but not with features.
    (
        boundaries_tables,
        boundaries,
        co_boundaries,
    ) = extract_boundaries_and_coboundaries_from_simplex_tree(simplex_tree, id_maps, complex_dim)
    # Note: complex_dim above is only 1 at this point.
    ...
    cochains = []
    # Since complex_dim is still 1 (??) this does not create cochains of dim 2.
    for i in range(complex_dim + 2):
...
def build_tables_with_rings(edge_index, simplex_tree, size, max_k):
    ...
    Returns
    -------
    cell_tables : list of lists
        Eg, cell_tables[0] gives the list of simplices of dim 0.
        Eg, cell_tables[2] gives the list of rings (of dim 2).
            Each ring is a list of nodes, called "cell" in this code.
    id_maps : list of dicts
        Eg, id_maps[0] gives the map from the node to its node id (trivial).
        Eg, id_maps[1] gives the map from the edge to its edge id (by counting them).
        Eg, id_maps[2] gives the map from the ring to its ring id (by counting them).
    """
...
def extract_boundaries_and_coboundaries_with_rings(simplex_tree, id_maps):
    ...
    simplex_tree : gudhi.SimplexTree
        The simplex tree of the graph or complex.
        It has nodes and edges (but not faces nor rings).
    id_maps : list of dicts
        Eg, id_maps[0] gives the map from the node to its node id (trivial).
        Eg, id_maps[1] gives the map from the edge to its edge id (by counting them).
        Eg, id_maps[2] gives the map from the ring to its ring id (by counting them).
        This one contains the rings.
    """
...
def compute_ring_2complex(
    ...
    Parameters
    ----------
    x : array-like, torch.tensor or np.array, shape=[n_nodes, n_node_features]
        The feature matrix for the nodes of the graph (shape [num_vertices, num_v_feats])
    edge_index: array-like, torch.tensor or np.array, shape=[2, n_edges]
        The edge_index of the graph (a tensor of shape [2, num_edges])
    ...
    (
        ...
        boundaries,
        co_boundaries,
    ) = extract_boundaries_and_coboundaries_with_rings(simplex_tree, id_maps)
    ...
    # print("len(shared_boundaries)", len(shared_boundaries))
    # print(shared_boundaries)
    # print("len(shared_coboundaries)", len(shared_coboundaries))
    # print(shared_coboundaries)
    # print("len(lower_idx)", len(lower_idx))
    # print(lower_idx)
    # print("len(upper_idx)", len(upper_idx))
    # print(upper_idx)
    ...
    for i in range(complex_dim + 1):
        y = v_y if i == 0 else None
        # print("generating cochain for dimension ", i)
        cochain = generate_cochain(
            i,
            xs[i],
            upper_idx,
...
def convert_graph_dataset_with_rings(
    ...
    """ "Lift graph to cell complex using rings.

    Parameters
    ----------
    dataset : InMemoryDataset
        The dataset to convert, using InMemoryDataset
        class from pytorch geometric.
    max_ring_size : int
        Maximum number of nodes defining a face.
    include_down_adj : bool
    """
    dimension = -1
    ...
    for c, complex in enumerate(complexes):
        ...
        #     print("Size of complex cochains is ", complex.cochains[1].x.size(0))
        #     print("Size of graph is ", (graph.edge_index.size(1) // 2))
        #     assert complex.cochains[1].x.size(0) == (graph.edge_index.size(1) // 2)

    return complexes, dimension, num_features[: dimension + 1]


def square_bdry_index(boundary_index):
    """
    ...
    """
    n_nodes = max(boundary_index[0]) + 1
    edge_index = [[], []]
    for n, e in zip(boundary_index[0], boundary_index[1]):
        edge_index[0].append(n)
        edge_index[0].append(e + n_nodes)
        edge_index[1].append(e + n_nodes)
