...
class CustomRayWandbCallback(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        self.fold_train_losses.append(train_loss)
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        if not trainer.sanity_checking:
            # Calculate the mean validation loss for the current epoch
            loss = torch.vstack(pl_module.val_losses).detach().cpu().mean()
            self.fold_val_losses.append(loss.item())  # Store the loss for this fold
            ...
            # Calculate and log mean and std if all folds are done
            if len(self.fold_val_losses) == 3:  # Assuming 3 folds
