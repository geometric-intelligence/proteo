...
class GAT(nn.Module):
    def __init__(self, opt, input_dim, omic_dim, label_dim, dropout, alpha):
        ...
        self.attentions1 = [
            GraphAttentionLayer(input_dim, self.nhids[0], dropout=dropout, alpha=alpha, concat=True)
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        a_input = torch.cat(
            [h.repeat(1, 1, N).view(bs, N * N, -1), h.repeat(1, N, 1)], dim=-1
        ).view(bs, N, -1, 2 * self.out_features)
