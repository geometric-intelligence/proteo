...
def get_sex_mutation_age_distribution(config):
    ...
    return train_sex_labels, test_sex_labels, train_mutation_labels, test_mutation_labels, train_age_labels, test_age_labels
...
def predict_for_subgroups_with_labels(relative_checkpoint_path, device, mean, std):
    ...
    train_preds, train_targets, _, val_preds, val_targets, _ = load_model_and_predict(module, config, device)
    ...
    for sex in sex_labels:
        for mutation in mutation_labels:
            for age_range in age_bins:
                ...
                mask = (train_sex_labels == sex) & (train_mutation_labels == mutation) & (train_age_labels >= age_range[0]) & (train_age_labels < age_range[1])
    ...
    for sex in sex_labels:
        for mutation in mutation_labels:
            for age_range in age_bins:
                ...
                # Create mask for current subgroup
                mask = (test_sex_labels == sex) & (test_mutation_labels == mutation) & (test_age_labels >= age_range[0]) & (test_age_labels < age_range[1])
